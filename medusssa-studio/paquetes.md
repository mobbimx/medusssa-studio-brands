# Medusssa Studio — Paquetes Comerciales 2026

> Documento cara al cliente. Precios en MXN, mercado Xalapa, Veracruz.
> El desglose interno de costos y márgenes está en `estructura-costos.md` (NO compartir con clientes).

---

## Filosofía de los paquetes

Dos tamaños, dos modalidades cada uno:

| | Modalidad Orgánica | Modalidad Impulso (con pauta) |
|---|---|---|
| **Para quién** | Negocios que quieren presencia constante y crecer sin invertir en anuncios todavía | Negocios que quieren resultados más rápidos: alcance, mensajes y clientes nuevos |
| **Qué incluye extra** | — | Pauta publicitaria en Meta (Facebook/Instagram) **ya incluida en el precio**, con configuración, segmentación y optimización |

La pauta **no se cobra aparte**: el cliente paga un solo precio mensual y Medusssa administra todo. Eso simplifica la venta ("un solo pago, todo incluido") y elimina la fricción de pedirle al cliente que ponga su tarjeta en Meta.

---

## 🪟 PAQUETE VITRINA — El escaparate digital mínimo

*Para el negocio muy pequeño (puesto, local familiar, emprendimiento que empieza) que hoy no publica nada o publica desde el celular sin diseño. Es el paquete más accesible de Medusssa y **funciona porque está sistematizado, no porque esté regalado**.*

### Incluye — **$1,700 / mes (con publicidad incluida)**

- **2 videos al mes** grabados y editados por nosotros (productos, promociones, precios, horarios o trends del momento)
- **Equipo profesional de grabación incluido**: dron, estabilizador, equipo de iluminación y micrófonos — calidad que ningún negocio logra con el celular
- **3 plantillas de historias al mes** listas para que el cliente las suba cuando quiera (productos, promociones, precios, horarios)
- **$300 de publicidad mensual en Meta ya incluida**: 1 campaña local (promoción del mes o mensajes a WhatsApp), configurada y administrada por nosotros
- **Todo el mes programado por adelantado**: el cliente aprueba una sola vez y se publica automático
- Resumen trimestral de métricas básicas

### Condiciones específicas de este paquete

- **1 visita de grabación al mes** (máx. 2 horas en sitio): los 2 videos se graban en la misma sesión, agendada en el día de grabación de la zona. Si el cliente cancela con menos de 24 h, la sesión se recorre al mes siguiente.
- **1 ronda de cambios sobre el lote del mes**, no pieza por pieza.
- **No incluye publicaciones de feed diseñadas, historias publicadas por nosotros ni community management.** Cualquiera de esas necesidades = paquete Presencia o Semilla.
- Solo con **pago trimestral anticipado ($5,100)** o semestral. Cobrar mes a mes un paquete de este tamaño cuesta más de lo que deja; la pauta se activa solo con el trimestre pagado.
- ¿Quiere más anuncios? Bloques extra de pauta: +$650 por cada $500 adicionales a Meta.

**Argumento de venta:** *"Por menos de $57 al día tienes videos con dron, luces y micrófonos profesionales — y publicidad para que los vea la gente de Xalapa. Todo en un solo pago."*

---

### 💼 Venta única: Kit de Arranque Digital — **$1,800 (pago único)**

Para el que ni siquiera puede comprometerse a una mensualidad: optimización completa de perfil + **10 plantillas editables en Canva** con su marca + calendario de 30 días con ideas y copys + mini guía de "cómo publicar". El cliente se autogestiona; nosotros cobramos una vez, sin carga recurrente. Es también un generador de prospectos: al mes, muchos regresan pidiendo que lo hagamos por ellos.

---

## ✨ PAQUETE PRESENCIA — Entrada para pequeñas empresas

*El negocio que apenas empieza con redes o que hoy no puede pagar más: quiere verse profesional y constante sin una gran inversión. **Precio ancla: menos de $2,000.***

### Incluye — **$1,900 / mes**

- **6 publicaciones al mes** (diseños gráficos alineados a los colores y logo del negocio)
- **2 reels al mes** editados con material que envía el cliente (fotos/videos de su celular)
- **Historias 2 veces por semana** (reciclaje de publicaciones, promociones, recordatorios)
- **Calendario de contenido mensual simple** aprobado antes de publicar
- **Optimización de perfil** el primer mes (foto, bio, destacadas, botón de WhatsApp)
- **Mini reporte mensual**: alcance, seguidores y mejor publicación

### Condiciones específicas de este paquete

- El **material crudo lo pone el cliente** (nosotros lo hacemos verse profesional); no incluye sesiones ni grabación.
- 1 ronda de cambios por pieza; entregas y respuestas en horario hábil (hasta 48 h).
- No incluye community management ni pauta.

### Add-on opcional: Chispa de pauta — **+$650 / mes**

$500 de inversión publicitaria en Meta ya administrada (1 campaña local de promoción o mensajes a WhatsApp). Se ofrece **a partir del segundo mes**, cuando el cliente ya vio su perfil transformado.

**Argumento de venta:** *"Por menos de $65 al día tienes a alguien profesional manejando tu imagen todos los días del mes."*

**Rol estratégico:** este paquete existe para **firmar rápido y subir después**. A los 2–3 meses, con resultados en la mano, la conversación natural es pasar a Semilla ($3,500) o Semilla Impulso ($4,900).

---

## 🌱 PAQUETE SEMILLA — Para negocios pequeños

*Nails studios, salones, consultorios, cafeterías, tiendas locales que necesitan verse profesionales y mantenerse activos.*

### Incluye (ambas modalidades)

- **10 publicaciones al mes** (feed: diseños gráficos + fotos editadas)
- **4 reels al mes** (edición de material que envía el cliente + 1 grabado por nosotros)
- **Historias 3 veces por semana** (reciclaje de contenido, encuestas, recordatorios)
- **Calendario de contenido mensual** aprobado por el cliente antes de publicar
- **Diseño alineado a identidad de marca** (si no tiene, mini-guía de identidad incluida el primer mes)
- **Reporte mensual simple**: alcance, seguidores, mejores publicaciones
- 1 ronda de cambios por pieza

### 🌱 Semilla Orgánico — **$3,500 / mes**

Crecimiento con contenido constante, hashtags locales, tendencias y colaboraciones sugeridas. Ideal para construir base y portafolio.

### 🌱 Semilla Impulso — **$4,900 / mes**

Todo lo del orgánico **+ pauta incluida**:

- **$1,000 de inversión publicitaria mensual en Meta** (ya dentro del precio)
- 2 campañas activas al mes (ej. promoción del mes + campaña de mensajes a WhatsApp)
- Segmentación local (Xalapa y zona de influencia)
- Optimización semanal y resultados en el reporte mensual

**Argumento de venta:** *"Por $1,400 más al mes, tu negocio deja de depender solo del algoritmo: llegamos directo a la gente de Xalapa que busca lo que ofreces."*

---

## 🚀 PAQUETE CRECIMIENTO — Para negocios en expansión

*Gimnasios, agencias de viajes, salones de eventos, marcas con más de un local o que ya venden y quieren escalar.*

### Incluye (ambas modalidades)

- **16 publicaciones al mes** (feed: diseño + fotografía)
- **8 reels al mes** con guión (aprovechando nuestra metodología de guiones probada)
- **Historias diarias** (lunes a sábado)
- **1 sesión de contenido mensual en sitio** (fotos + video para todo el mes)
- **Estrategia trimestral de contenido** con pilares definidos por marca
- **Community management básico**: respuesta a comentarios y filtrado de mensajes a WhatsApp (horario hábil)
- **Reporte quincenal** con análisis y ajustes
- 2 rondas de cambios por pieza
- Junta mensual de resultados (presencial o videollamada)

### 🚀 Crecimiento Orgánico — **$7,500 / mes**

Para marcas que quieren dominar el contenido orgánico: reels con guión, tendencias, colaboraciones y comunidad activa.

### 🚀 Crecimiento Impulso — **$9,900 / mes**

Todo lo del orgánico **+ pauta incluida**:

- **$2,000 de inversión publicitaria mensual en Meta** (ya dentro del precio)
- Hasta 4 campañas activas (alcance, mensajes, seguidores, promociones)
- Públicos personalizados y retargeting (visitantes del perfil, interacciones)
- Optimización semanal + recomendaciones de escalamiento en cada reporte

**Argumento de venta:** *"Tu contenido ya trabaja; la pauta lo multiplica. Todo administrado por nosotros, un solo pago."*

---

## Comparativa rápida (para enviar por WhatsApp)

| | 🪟 Vitrina | ✨ Presencia | 🌱 Semilla Orgánico | 🌱 Semilla Impulso | 🚀 Crecimiento Orgánico | 🚀 Crecimiento Impulso |
|---|---|---|---|---|---|---|
| Publicaciones/mes | — | 6 | 10 | 10 | 16 | 16 |
| Videos/reels al mes | 2 (grabados por nosotros) | 2 (material del cliente) | 4 | 4 | 8 (con guión) | 8 (con guión) |
| Historias | 3 plantillas (las sube el cliente) | 2×semana | 3×semana | 3×semana | Diarias | Diarias |
| Grabación propia | 1 visita/mes con equipo pro (dron, luces, micrófonos) | — | 1 reel/mes | 1 reel/mes | Sesión 1/mes | Sesión 1/mes |
| Community mgmt | — | — | — | — | ✔ | ✔ |
| Pauta incluida | $300/mes | Opcional +$650 | — | $1,000/mes | — | $2,000/mes |
| Reportes | Trimestral básico | Mensual simple | Mensual | Mensual | Quincenal | Quincenal |
| **Precio** | **$1,700** (pago trimestral) | **$1,900** | **$3,500** | **$4,900** | **$7,500** | **$9,900** |

---

## Condiciones comerciales

- **Permanencia mínima: 3 meses.** El contenido y la pauta necesitan tiempo para dar resultados; también protege el flujo de Medusssa. Sin plazo forzoso después del tercer mes (aviso con 15 días).
- **Pago por adelantado** los primeros 5 días de cada mes. La pauta se activa solo con pago recibido.
- **Pago trimestral anticipado: 10% de descuento** (mejora tu flujo de caja inmediato — priorizar en la venta este mes).
- Lo no incluido (sesiones extra, campañas especiales, diseño de menú/catálogo) se cotiza aparte.

### Ruta de crecimiento del cliente (upsell natural)

```
Kit de Arranque → Vitrina → Presencia → Semilla Orgánico → Semilla Impulso → Crecimiento Orgánico → Crecimiento Impulso
```

Cada 3 meses se presenta el reporte trimestral con la recomendación del siguiente paso. Un cliente que ve resultados con $1,000 de pauta es el candidato natural para subir de paquete, y siempre puede **aumentar solo la pauta** en bloques de +$500 (se cobran +$650 por bloque, ver documento interno).

---

*Medusssa Studio | Xalapa, Veracruz*
